  ...
  return (
    <div className="overflow-hidden text-ungu-400 bg-white">
      <Helmet>
        ...
        <meta
          name="keywords"
          content="Public Action, UGM, Public policy, public management"
        ></meta>
      </Helmet>
      <div className="h-screen overflow-hidden bg-putih flex items-center relative">
        <div className="mx-auto z-40 relative z-10">
          <h1 className="text-ungu-200 text-6xl md:text-7xl font-serif font-black text-center">
            {" "}
            Welcome to{" "}
          </h1>
          <img
            src={process.env.PUBLIC_URL + "images/logo-pa-icon.png"}
            alt="logo public action"
            className="h-96 mx-auto "
          />
        </div>
        <div
          style={{
            backgroundImage: `url(${process.env.PUBLIC_URL}/svg/pattern1.svg)`,
          }}
          className="absolute z-0 w-full h-full opacity-50"
        ></div>
        <div className="absolute z-10 md:w-80 md:h-80 w-52 h-52 bg-gradient-to-br from-ungu-100 to-ungu-400 rounded-full -top-20 md:-left-12 -left-20">
          {" "}
        </div>
        <div className="absolute hidden md:block z-10 md:w-52 md:h-80 w-44 h-64 bg-ungu-200 opacity-75 rounded-full -top-20 -right-16">
          {" "}
        </div>
        <div className="absolute hidden md:block z-10 md:w-72 md:h-56 w-56 h-52 bg-ungu-200 opacity-75 py-2 rounded-full md:top-20 -top-10 -right-24">
          {" "}
        </div>
      ...
      <div>
        <div
          className="relative h-96 flex flex-col justify-center items-center font-sans text-center bg-contain md:bg-auto bg-opacity-35"
          style={{
            backgroundImage: `url(${process.env.PUBLIC_URL}/svg/topography.svg)`,
          }}
        >
          <p className="font-bold text-4xl mb-10 text-ungu-400 z-20 font-serif">
            {" "}
            Our Grand Theme{" "}
          </p>
          <p className="text-lg max-w-xl text-ungu-300 z-20 font-semibold px-2">
            {" "}
            "Innovative Youths in Action: Emphasizing Public Value in Addressing
            Perpetual Social Issues”{" "}
          </p>
        ...
        <div className="h-1/2 bg-ungu-400 my-auto pt-8 pb-12">
          <h2 className="text-center text-4xl font-bold font-serif text-white pt-4 mb-12 md:mb-0">
            Abstract Submission for AIPC will be closed in
          </h2>
          <Countdown isDark={true} date="aug 31, 2021 23:59:59" />
        </div>
      </div>
      <div className="bg-ungu-100 justify-center py-5">
        <h2 className="text-4xl text-ungu-400 font-bold text-center mb-5 font-serif">
          Our Pre-Event Projects{" "}
        </h2>
        <iframe
      ...
      <div
        className="py-12 md:px-20 px-8 bg-opacity-50"
        style={{
          backgroundImage: `url(${process.env.PUBLIC_URL}/svg/pattern2.svg)`,
        }}
      >
        <h2 className="text-4xl text-ungu-400 font-serif font-bold text-center my-8">
          Our Events
        </h2>
      ...
      <div className="bg-putih py-16 px-12 md:px-16" id="partners">
        <div className="my-5 md:mx-16 mx-0">
          <p className="text-ungu-400 font-bold text-4xl text-center mb-6 font-serif">
            Part of
          </p>
